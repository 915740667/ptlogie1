...
import com.ptlogie.service.gatewayService;
...
public class GatewayController {
	...
	@Autowired
	private gatewayService gatewaySrv;
	@Autowired
	private Page page;
	
	@RequestMapping("/gatewayList")
	   public String  relayList(Model model){
		List<Gateway> list =gatewaySrv.findAll();
		page.setTotalCounts(list.size());
		page.setCurrentPage(1);
		page.getTotalPages();
		List<Gateway> dataList=gatewaySrv.findByPage(0,page.getPageSize());
	...
	public Page findByPage(Model model,int pageNum){
		...
		List<Gateway> dataList=gatewaySrv.findByPage(startObj,page.getPageSize());
		page.setDataList(dataList);
		return page;
	}
//	@RequestMapping("/add")
...
//	}
	...
	public double  eqpOnline() throws IOException{
		List<Gateway> list =gatewaySrv.findAll();
		List<Gateway> onlineList =gatewaySrv.findOnline();
